...
test("setupPlan maps each setup state to its fix, worst problem first", () => {
  ...
  assert.equal(signIn.launchCommand,
    "basecamp auth login; rc=$?; omarchy-shell -q 37signals.basecamp refresh; (exit $rc)")
  ...
  assert.equal(install.launchCommand,
    "omarchy-pkg-add basecamp-cli && basecamp auth login; rc=$?; omarchy-shell -q 37signals.basecamp refresh; (exit $rc)")
  ...
  assert.equal(update.launchCommand,
    "omarchy update; rc=$?; omarchy-shell -q 37signals.basecamp refresh; (exit $rc)")
