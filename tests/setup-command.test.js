...
function setupFixture(run) {
  const runtimeDir = mkdtempSync(path.join(tmpdir(), "basecamp-setup-test-"))
...
test("setup completion runs after command failure and preserves its status", async () => {
  await setupFixture(({ completionLog, env }) => {
    for (const exitCode of [42, 130]) {
      ...
      const result = spawnSync("bash", ["-c", Model.setupLaunchCommand("exit " + exitCode, "37signals.basecamp")], {
        encoding: "utf8",
        env
      })

      assert.equal(result.status, exitCode, result.stderr)
      assert.equal(readFileSync(completionLog, "utf8").trim(), "-q 37signals.basecamp setupFinished")
    }
  })
})
...
test("setup lock blocks concurrent runs and recovers after terminal termination", async () => {
  await setupFixture(async ({ runtimeDir, completionLog, env }) => {
    const command = Model.setupLaunchCommand("sleep 30", "37signals.basecamp")
    const first = spawn("bash", ["-c", command], { detached: true, env, stdio: "ignore" })
    const lockPath = path.join(runtimeDir, "37signals.basecamp.setup.lock")
